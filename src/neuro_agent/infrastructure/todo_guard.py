...
Mirrors deep_agents_from_scratch/todo_guard.py.

Provides a TodoGuardMiddleware that hooks into the LangGraph agent loop
via the `after_model` lifecycle hook. Enforces the core principle:
...
from neuro_agent.infrastructure.execution_tracker import check_plan_complete
...
class TodoGuardMiddleware(AgentMiddleware[AgentState, Any]):
    """Middleware that enforces plan-first execution.

    Core principle: Plan → Execute → Complete.
    ...
    def _check_todos(self, state: AgentState) -> Union[dict[str, Any], Command, None]:
